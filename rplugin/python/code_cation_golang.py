...
def cache_k8s_module(cache_file):
    with open(cache_file, "r") as fp:
        ret = [_.lstrip("./staging/src/") for _ in fp.read().split() if "./staging/src/" in _]
...
class CodeActionGolangK8s(object):
    def __init__(self, nvim:Nvim):
        ...
        self.cache_dir = os.path.join(Path.home(), ".config/nvim/rplugin/python/cache/k8s")
    ...
    def gather_all_k8s_modules(self, k8s_version):
        ...
        cache_file = os.path.join(self.cache_dir, "{}/go.mod".format(k8s_version))
        if os.path.exists(cache_file):
            return cache_k8s_module(cache_file=os.path.join(self.cache_dir, "{}/go.mod".format(k8s_version)))
        try:
            response = urlopen(self.mod_api.format(k8s_version), timeout=20).read().decode("utf-8")
    ...
    def go_mod_add_pkg(self, mod_name, version):
        cache_file = os.path.join(self.cache_dir, "{}/{}.json".format(version, mod_name.split("/")[-1]))
        if os.path.exists(cache_file):
            json_data = cache_k8s_mod_info(cache_file)
        else:
            try:
                cmd = 'go mod download -json "{}@kubernetes-{}"'.format(mod_name, version)
        ...
        cmd = 'go mod edit "-replace={MOD_NAME}={MOD_NAME}@{VERSION}"'.format(MOD_NAME=mod_name, VERSION=version)
